...
**DICOM** (*Digital Imaging and Communications in Medicine*) nace del comité conjunto **ACR-NEMA** (creado en 1983; primera versión del estándar, ACR-NEMA 300, en **1985**; la tercera, de **1993**, adopta el nombre DICOM y es la base de la actual). Lo mantiene NEMA/MITA como la serie **PS3** (una veintena de partes), con **varias ediciones al año** (edición vigente: **2026**); está adoptado como **ISO 12052**. Define a la vez el **formato de los objetos** (imágenes y metadatos), los **servicios de red** para intercambiarlos y el **soporte físico** de almacenamiento.

- **Partes clave de la serie PS3**: PS3.3 (definiciones de objetos, IOD), PS3.4 (especificación de las clases de servicio), PS3.5 (estructura de datos y codificación), PS3.6 (diccionario de datos), PS3.10 (soportes y formato de fichero) y PS3.18 (servicios web, DICOMweb).
...
- **Objetos no imagen**: DICOM también define objetos para el **informe estructurado** (DICOM SR: mediciones, informes de dosis de radiación), los **estados de presentación** (GSPS: anotaciones y ajustes de visualización reproducibles) y los documentos encapsulados (PDF).

Las clases de servicio principales (PS3.4):

| Clase de servicio | Función |
| --- | --- |
| **Verification** | Comprobar la comunicación (C-ECHO) |
| **Storage** | Almacenar objetos en otro nodo (C-STORE) |
| **Storage Commitment** | Confirmar que el archivo asume la custodia del objeto |
| **Query/Retrieve** | Buscar y recuperar estudios (C-FIND, C-MOVE/C-GET) |
| **Modality Worklist** | Descargar la lista de trabajo desde el RIS (C-FIND) |
| **MPPS** | Notificar la realización de la exploración (N-CREATE/N-SET) |
| **Print Management** | Impresión en placa (servicios N) |

## Intercambio de mensajes: los servicios DIMSE

Sobre una asociación de red DICOM, los equipos intercambian comandos **DIMSE** (*DICOM Message Service Element*), divididos en **DIMSE-C** (servicios compuestos, los C-\*, que operan sobre conjuntos de datos) y **DIMSE-N** (servicios normalizados, los N-\*, que operan sobre instancias de objetos concretos):
...
| **N-CREATE, N-SET…** | Servicios normalizados: MPPS, impresión, gestión de contenido |

- **La asociación**: cada nodo es una **entidad de aplicación** (AE, identificada por su *AE Title*, junto a IP y puerto); al abrirse la asociación se negocian los **contextos de presentación**: qué clase SOP se usará (sintaxis abstracta) y con qué sintaxis de transferencia.
- **Flujo de trabajo**: la **Modality Worklist (MWL)** permite a la modalidad descargar del RIS la lista de pacientes citados con sus datos demográficos (una consulta C-FIND; evita teclearlos y garantiza identificadores correctos); el **MPPS** (*Modality Performed Procedure Step*) comunica el inicio y fin real de cada exploración.
- **Sintaxis de transferencia**: define cómo se codifican los datos: la representación de valores (**VR**, el tipo de cada atributo) **implícita o explícita**, el orden de bytes y la **compresión** (JPEG, JPEG 2000, JPEG-LS, RLE), con o sin pérdida según el uso diagnóstico.
...
El precursor fue **WADO-URI** (recuperación de una imagen por URL); la familia incluye también **UPS-RS** (listas de trabajo unificadas sobre web). DICOMweb convive con DIMSE: dentro del hospital domina DIMSE; hacia el exterior (portales de paciente, telerradiología, integración con la HCE) se imponen los servicios web.
...
- **Integración RIS-PACS-HIS**: el ciclo completo de una exploración (perfil IHE **Scheduled Workflow**, tema [92](92-normalizacion-en-informatica-sanitaria.md)) encadena los tres sistemas:
    ...
    2. El RIS recibe la petición, le asigna el **número de acceso** (*accession number*) y **cita** al paciente.
...
- **Clave de trazabilidad**: el **número de acceso** (`(0008,0050)`), asignado por el RIS a la petición y copiado en las imágenes DICOM y en el informe HL7, es lo que une los tres sistemas: garantiza que petición, estudio, imágenes e informe hablan del mismo acto asistencial.
