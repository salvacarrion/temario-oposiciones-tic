...
**APIs de la plataforma** (accesibles desde JavaScript): almacenamiento local (**Web Storage**: `localStorage` persistente y `sessionStorage` por pestaña), **geolocalización**, **Web Workers** (hilos en segundo plano), **History API** (navegación de las SPA), arrastrar y soltar, y **WebSocket** (tema 52).
...
- **Media queries** (`@media (max-width: 768px) { ... }`): aplican reglas según las características del dispositivo (ancho, orientación, esquema de color); son la base del **diseño web adaptativo** (tema 54).
...
**DOM y eventos**: el navegador expone el documento como árbol de objetos (**DOM**) manipulable desde JavaScript; los manejadores de eventos reaccionan a las acciones del usuario. **JSON** (`JSON.parse` / `JSON.stringify`) es el formato de intercambio nativo (tema 56).
...
Las aplicaciones web actuales se construyen con frameworks de **componentes** reutilizables con estado. Los tres principales son Angular, React y Vue; Node.js lleva JavaScript al servidor. Los modelos de aplicación que habilitan (SPA, renderizado en cliente y servidor) se tratan en el tema 52.
...
- **React** (Meta, 2013, código abierto): **biblioteca** de interfaces de usuario, no framework completo: se centra en la capa de vista.
    ...
    - **Ecosistema**: enrutado (React Router), estado global (Redux, Zustand), metaframework **Next.js**; **React Native** para móvil (tema 55).
