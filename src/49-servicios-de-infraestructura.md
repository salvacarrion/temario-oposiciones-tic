...
- **IIS (Internet Information Services)**: el servidor web de Windows, administrado por consola o PowerShell, con aislamiento por **grupos de aplicaciones** (*application pools*, con identidad propia y **reciclaje** periódico), jerarquía servidor → sitios → aplicaciones, configuración por sitio en `web.config`, e integración con el ecosistema .NET y la **autenticación integrada de Windows** (Kerberos/NTLM, tema [48](48-administracion-de-sistemas-windows-y-directorio-activo.md)).
...
El equivalente en Apache es el **host virtual**:

```apache
<VirtualHost *:443>
    ServerName app.gva.es
    SSLEngine on
...
- **Proxy directo**: intermedia las salidas de los usuarios hacia Internet: control de acceso y filtrado de contenidos, caché y registro de navegación. El referente libre es **Squid** (puerto por defecto **3128**; jerarquías de caché e integración con autenticación corporativa), gobernado por **ACL** en `squid.conf`: `acl redlocal src 10.0.0.0/8` + `http_access allow redlocal` (y denegación final implícita).
...
- **Administración del MTA**: en **Postfix**, configuración en `main.cf`, cola visible con `postqueue -p` y reglas de reenvío estrictas (nunca **open relay**: solo se reenvía lo autenticado o lo de las propias redes); en **Exchange**, roles de **buzón** y de **transporte perimetral** (Edge, en la DMZ), conectores de envío/recepción y consola EAC, con la libreta global (**GAL**) alimentada por el directorio.
